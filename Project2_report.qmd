...
pr.out <- prcomp(plasma_df, scale = FALSE)
pr.out$rotation

pr.var <- pr.out$sdev^2
pve <- pr.var / sum(pr.var)

plasma_pca_df <- data.frame(pve=pve,PC=1:13)

plasma_pca_df %>% ggplot(aes(x=PC,y=pve))+
  geom_point(size=3)+
  geom_line() + 
  labs(y="Proportion of variance explained") 

plasma_df_with_pca <- bind_cols(plasma_df, pr.out$x[,1:3])

pca_lm <- lm(log(betaplasma) ~ PC1 + PC2 + PC3, data=plasma_df_with_pca)
summary(pca_lm)
...
#| out-width: "80%"
plot(cvrf_obj$predicted[[bestvars]], y, 
