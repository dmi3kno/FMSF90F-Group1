...
@fig-pcr-validplot1 shows the out-of-fold (cross-validated) MSEP for each number of components in the PCR.
...
The number of principal components suffice to explain most of the variability in the data, as well as the relationship with the response belong to 8 components. Let's perform CV on the training dataset and see if we can reduce the model even further. 
...
The figure @fig-pcr-validplot2 indicates that performing PCR with 3 as number of components can results in substantial improvement over least squares. Thus, we select the lowest number of components which minimizes MSE.
...
In conclusion, All three methods offer some improvement over least squares. However, PCR and ridge regression slightly outperform the lasso regression. Thus, although PCR tends to work exceptionally well in many practical settings, this method only allows models to be developed based on a small set of features. Thus, PCR is more closely related to ridge regression than to lasso.
...
Now we tune the spline model changing the degrees of freedom and measuring the out-of-fold error. @fig-ns-cv-error shows ten-fold cross-validated mean squares errors for splines with various degrees of freedom fit to the data.
...
Figure @fig-ghg-ns38 compares a natural cubic spline with 3 degrees of freedom, purple line, to a degree-8 polynomial, red line, on the data set. The flexibility in the degree-8 polynomial produces ... 
