...
`glmnet` requires a model-matrix as the model specification.
 A model matrix is a matrix with a column for each predictor (i.e. $x$-variable) in the model (including the extra columns for polynomials, dummy variables etc), and each row is for a unique observation.
