...
In this assignment we will use the test AUC (area under the receiver operator curve) to assess the performance and compare the models. We will also calculate some classification performance metrics from the confusion matrix, which, for two-class problems shows true positives (TP), true negatives (TN), false positives (FP) and false negatives (FN). Using these totals we will compute 

$$Accuracy=\frac{TP+TN}{TP+TN+FP+FN}$$

$$Sensitivity=\frac{TP}{P}=\frac{TP}{TP+FN}$$

Sensitivity is also called the true positive rate or recall.


$$Specificity=\frac{TN}{N}=\frac{TN}{TN+FP}$$

Specificity is sometimes called true negative rate.

Note that these metrics are computed conditional on the selected cutoff. Therefore, we inspect the `pROC` object and identify the optimial cutoff for each model and then apply this cutoff to each of the class probability outputs and compute the metrics. 
...
glm_roc <- roc(bigfoot$class[test_idx],
               preds_glm)
glm_metrics <- coords(glm_roc,"best")
ggroc(glm_roc, legacy.axes = TRUE)+
  geom_abline(slope=1, intercept = 0, color="grey70")

```

The AUC for the logistic regression is `r auc(glm_roc)`. The best cutoff is at `r glm_metrics$threshold`. The sensitivity (TPR) is `r glm_metrics$sensitivity` and specificity (TNR) is `r glm_metrics$specificity`.
...
lasso_metrics <- coords(lasso_roc,"best")
ggroc(lasso_roc, legacy.axes = TRUE)+
  geom_abline(slope=1, intercept = 0, color="grey70")
```

The AUC for the tuned LASSO regression is `r auc(lasso_roc)`, which is not that much different from the logistic regression. The best cutoff is at `r lasso_metrics$threshold`. The sensitivity (TPR) is `r lasso_metrics$sensitivity` and specificity (TNR) is `r lasso_metrics$specificity`.
...
lda_roc <- roc(bigfoot$class[test_idx],
                lda_preds)
lda_metrics <- coords(lda_roc,"best")
ggroc(lda_roc, legacy.axes = TRUE)+
  geom_abline(slope=1, intercept = 0, color="grey70")
```

The AUC for LDA is a meager `r auc(lda_roc)`. The best cutoff is at `r lda_metrics$threshold`. The sensitivity (TPR) is `r lda_metrics$sensitivity` and specificity (TNR) is `r lda_metrics$specificity`.
...
rf_roc <- roc(bigfoot$class[test_idx],
                rf_preds)
rf_metrics <- coords(rf_roc,"best")
ggroc(rf_roc, legacy.axes = TRUE)+
  geom_abline(slope=1, intercept = 0, color="grey70")
```

The AUC for the random forest model is `r auc(rf_roc)`. The best cutoff is at `r rf_metrics$threshold`. The sensitivity (TPR) is `r rf_metrics$sensitivity` and specificity (TNR) is `r rf_metrics$specificity`.
...
svm_lin <- svm(class ~ ., data = bigfoot, subset=train_idx,
              kernel = "linear", cost = .01, scale = FALSE, probability=TRUE)
svm_lin_preds <- attr(predict(svm_lin, newdata=bigfoot[test_idx,],  
                              probability=TRUE), "probabilities")[,"Class A"]
```
...
svmlin_metrics <- coords(lin_roc,"best")

ggroc(lda_roc, legacy.axes = TRUE)+
  geom_abline(slope=1, intercept = 0, color="grey70")
```
The AUC for the linear-kernel SVM `r auc(lin_roc)`. The best cutoff is at `r svmlin_metrics$threshold`. The sensitivity (TPR) is `r svmlin_metrics$sensitivity` and specificity (TNR) is `r svmlin_metrics$specificity`.

Let's try radial kernel

```{r}
svm_rad <- svm(class ~ ., data = bigfoot, subset=train_idx, probability=TRUE,
              kernel = "radial", gamma=0.01, cost = .01, scale = FALSE)
svm_rad_preds <- attr(predict(svm_rad, newdata=bigfoot[test_idx,],  
                              probability=TRUE), "probabilities")[,"Class A"]
```
...
svm_rad_roc <- roc(
   as.numeric(bigfoot$class[test_idx]),
   as.vector(svm_lin_preds))
svm_rad_metrics <- coords(svm_rad_roc,"best")

ggroc(svm_rad_roc, legacy.axes = TRUE)+
  geom_abline(slope=1, intercept = 0, color="grey70")
```

The AUC for the radial-kernel SVM `r auc(svm_rad_roc)`. The best cutoff is at `r svm_rad_metrics$threshold`. The sensitivity (TPR) is `r svm_rad_metrics$sensitivity` and specificity (TNR) is `r svm_rad_metrics$specificity`.

We can tune the SVM parameters (both `gamma` and `cost`).
...
```{r}
svm_bestrad_preds <- attr(predict(svm_bestrad_mod, newdata=bigfoot[test_idx,], 
                                  probability=TRUE), "probabilities")[,"Class A"]
```
...
svm_bestrad_metrics <- coords(svm_bestrad_roc,"best")
ggroc(svm_bestrad_roc, legacy.axes = TRUE)+
  geom_abline(slope=1, intercept = 0, color="grey70")
```

The AUC for the tuned radial SVM is `r auc(svm_bestrad_roc)`. The best cutoff is at `r svm_bestrad_metrics$threshold`. The sensitivity (TPR) is `r svm_bestrad_metrics$sensitivity` and specificity (TNR) is `r svm_bestrad_metrics$specificity`.

## Conclusion

In this section we considered several classification algorithms. The summary of the performance measured on the test set is provided in @tbl-auc-results. Based on AUC, the best performing algorithm is the Random Forest, which after tuning showed the AUC of `r auc(rf_roc)`, beating both SVM and the linear model. Therefore we pick Random Forest as out model of choice for the Bigfoot prediction challenge.

```{r}
#| label: tbl-auc-results
#| tbl-cap: "Test AUC for Bigfoot example per model"
#| echo: false
tibble::tibble(
  model=c("GLM", "LASSO", "LDA",
          "RF", "SVM linear", "SVM radial",
          "SVM radial tuned"),
  AUC=c(GLM=auc(glm_roc), Lasso=auc(lasso_roc), LDA=auc(lda_roc),
        RF=auc(rf_roc), SVMlin=auc(lin_roc), SVMrad=auc(svm_rad_roc),
        SVMtunedrad=auc(svm_bestrad_roc)),
  Cutoff=c(glm_metrics$threshold, lasso_metrics$threshold, lda_metrics$threshold,
           rf_metrics$threshold, svmlin_metrics$threshold, svm_rad_metrics$threshold,
           svm_bestrad_metrics$threshold),
  Sensitivity=c(glm_metrics$sensitivity, lasso_metrics$sensitivity, lda_metrics$sensitivity,
           rf_metrics$sensitivity, svmlin_metrics$sensitivity, svm_rad_metrics$sensitivity,
           svm_bestrad_metrics$sensitivity),
  Specificity=c(glm_metrics$specificity, lasso_metrics$specificity, lda_metrics$specificity,
           rf_metrics$specificity, svmlin_metrics$specificity, svm_rad_metrics$specificity,
           svm_bestrad_metrics$specificity)
) %>% 
