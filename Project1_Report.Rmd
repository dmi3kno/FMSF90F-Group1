...
# Approach 1: assumed the data to be independent and identically distributed 
...
```{r}
par(mar=c(4,4,4,4))
plot(acf(na.omit(icecore$EDML), plot=FALSE), main='EDML data')
```
...
EDML_dat <- icecore %>% 
  select(-GRIP) %>%
  filter(!is.na(EDML)) %>%
  group_by(row_number() %/% 3) %>%
  summarise_all(mean)
# Summary
summary(EDML_dat[-c(1,2)])
```
...
We select Q_stra_mid to build a simple linear regression to predict the EDML data. The reasons are that (1) it highly correlated to EDML and (2) a lot of 10Be is produce here.

```{r}
q_stra_mid_lm <- lm(EDML ~ Q_stra_mid, data=EDML_dat)
summary(q_stra_mid_lm)
```
The independent variable is significant and can explain ~78% of the variation as indicated by R-squared. Below are the confidence intervals for the coefficient estimates.
...
The response variable is plotted versus the independent variable below. The plot includes the linear prediction (black solid line), its confidence interval (grey envelope) and its prediction interval (dashed black lines).
...
The linear model works quite well. Let's assess the model fit:
There is no time autocorrelation or trend in the residual.
...
```{r}
par(mar=c(4,4,4,4))
plot(acf(EDML_pred_simple$res, plot=FALSE),main="Residual (Simple Linear)")
```

The variance of error terms seems fine.

```{r}
ggplot(data=EDML_pred_simple, aes(x=fit,y=res)) + geom_point() + geom_smooth()
```
```{r}
ggplot(data=EDML_pred_simple, aes(x=fit,y=rstudent)) + geom_point() + geom_smooth()
```
...
# Fit
q_stra_mid_poly <- lm(EDML ~ Q_stra_mid + Q_stra_mid2, data=EDML_pred_simple)
...
ggplot(data=EDML_pred_simple, aes(x=Q_stra_mid,y=EDML)) + 
  ...
  geom_line(data=conf_int,aes(x=Q_stra_mid,y=upr), color='grey') + 
  geom_line(data=conf_int,aes(x=Q_stra_mid,y=lwr), color='grey') +  
  geom_line(data=pred_int,aes(x=Q_stra_mid,y=upr), color='black', linetype=2) + 
  geom_line(data=pred_int,aes(x=Q_stra_mid,y=lwr), color='black', linetype=2) +
  theme_bw() 
```
...
Let's try to use all of the variables to predict EDML (except age and index of course)

```{r}
all_lm <- lm(EDML ~ ., data=EDML_dat[-c(1,2)])
summary(all_lm)
```
...
```{r}
par(mar=c(4,4,4,4))
plot(acf(EDML_pred_multi$res, plot=FALSE),main="Residual (Simple Linear)")
```
```{r}
ggplot(data=EDML_pred_multi, aes(x=fit,y=res)) + geom_point() + geom_smooth()
```

```{r}
ggplot(data=EDML_pred_multi, aes(x=fit,y=rstudent)) + geom_point() + geom_smooth()
```
...
```{r}
pca_dat <- EDML_dat %>%
  select(Q_stra_low,Q_stra_mid,Q_stra_high,Q_trop_low,Q_trop_mid,Q_trop_high)
...
```{r}
ggplot(data=EDML_dat, aes(x=PC2,y=EDML)) + geom_point() + geom_smooth(method='lm')
```
...
```
```{r}
ggplot(data=EDML_pred_pca, aes(x=fit,y=res)) + geom_point() + geom_smooth()
```
```{r}
ggplot(data=EDML_pred_pca, aes(x=fit,y=rstudent)) + geom_point() + geom_smooth()
```

# Approach 2: assumed the data to be time-indexed
